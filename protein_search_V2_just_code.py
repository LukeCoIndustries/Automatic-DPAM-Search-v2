...
def Protein_search(File,Uniprot_col,run,Output_name,extra = None, Extra_col=None, Extra_name=None, Zscore=None,Protein_count=None):
    print('starting')
    your_data_df = pd.read_excel(File)
    ...
    print(len(processing_data['ID']))
    print(len(processing_data['arch']))
    print(len(processing_data['Code']))
    print(len(processing_data['x']))
    print(len(processing_data['h']))
    print(len(processing_data['t']))
    if extra in ['yes','YES','Yes','yEs','yeS']:
    ...
    with open("templates\summary.html",'w') as sumfile:
        print('Making a summary')
        sumfile.write("<h1>Summary</h1>")
        summary = {}
        for arch in processing_data['arch']:
            summary[arch] = processing_data['arch'].count(arch)
        for arch in summary.keys():
            sumfile.write( "<br><hw>"+str(arch)+ ": " +str(round(((summary[arch])/len(processing_data['arch'])*100),2)) + "%</hw>")
        ...
        sumfile.write('<br><br><h2>Ranking</h2>')
        sumfile.write('<br></hw>1st is '+str(rank['rank'][0])+" with " + str(rank[rank['rank'][0]])+"</hw>") 
        decode(rank['rank'][0])
        sumfile.write('<br><hw>2nd is '+str(rank['rank'][1])+" with " + str(rank[rank['rank'][1]])+"</hw>") 
        decode(rank['rank'][1])
        sumfile.write('<br><hw>3rd is '+str(rank['rank'][2])+" with " + str(rank[rank['rank'][2]])+"</hw>")
        decode(rank['rank'][2])
        sumfile.write('<br><hw>4th is '+str(rank['rank'][3])+" with " + str(rank[rank['rank'][3]])+"</hw>")
        decode(rank['rank'][3])
        sumfile.write('<br><hw>5th is '+str(rank['rank'][4])+" with " + str(rank[rank['rank'][4]])+"</hw>")
        decode(rank['rank'][4])
